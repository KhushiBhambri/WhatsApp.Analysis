...
if(data!=""):
    if(df.shape[0]==0):
        ...
        with col1:       
            st.markdown('<div style="display:flex;justify-content:space-between"><h4>Total Messages</h4><h4>{}</h4></div>'.format(str(messages)),unsafe_allow_html=True)
            st.markdown('<div style="display:flex;justify-content:space-between"><h4>Total Words</h4><h4>{}</h4></div>'.format(str(words)),unsafe_allow_html=True)
        ...
        with col2:
            fig,ax=plots.BarPlot(["MS","LS","ES","DM"],[media,len(links),emojies.shape[0],deleted],[0,0],orient='h',figsize=[8,6])
        ...
        if(timeline=='Daily'):
            st.markdown('<h6 style="color:{};text-align:center;">Average Messages per Day: {}</h6>'.format( subheadcolor,str(round(datetimeline['Messages'].sum()/datetimeline.shape[0]))),unsafe_allow_html=True)
            fig=plots.TimePlot(datetimeline,x='Date',y='Messages')
            st.plotly_chart(fig, config=config)
        if(timeline=='Weekly'):
            st.markdown('<h6 style="color:{};text-align:center;">Average Messages per Week : {}</h6>'.format( subheadcolor,str(round(weektimeline['Messages'].sum()/weektimeline.shape[0]))),unsafe_allow_html=True)
            fig = plots.TimePlot(weektimeline, x='Week-Number', y="Messages")
            st.plotly_chart(fig, config=config)
        if(timeline=='Monthly'):
            st.markdown('<h6 style="color:{};text-align:center;">Average Messages per Month : {}</h6>'.format( subheadcolor,str(round(monthtimeline['Messages'].sum()/monthtimeline.shape[0]))),unsafe_allow_html=True)          
